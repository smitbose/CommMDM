...
public class MessageEventGenerator implements EventQueue {
	...
	/** Multicast enable namespace -setting id ({@value}). */
	public static final String MULTICAST = "is_multicast";//sahil: Multicast boolean
	/** number of destination paramenter -setting id ({@value}). Denotes
	 *  the number of destinations a message will contain
	 */
	public static final String MULTICAST_DEST_S = "nrofDestination";
	...
	/** Denotes whether multicast is enabled or not */
	private boolean is_multicast;//multicast boolean
	/** Denotes the number of destinations for each message */
	...
	public MessageEventGenerator(Settings s){
		...
		this.is_multicast = s.getBoolean(MULTICAST);
		this.destinations = s.getInt(MULTICAST_DEST_S);
	...
	protected int drawToAddress(int hostRange[], int from) {
        ...
        //Sahil: multicast feature
        protected int [] multicast_destinations(int hostRange[],int from)
        {
        /*int size =hostRange[1]-hostRange[0];
        int message_destinations = rng.nextInt(size-5)+5;*/
	...
	public ExternalEvent nextEvent() {
		...
		mce = new MessageCreateEvent(from, to, this.getID(), 
                ...
                mce = new MessageCreateEvent(from, to, this.getID(), 
				msgSize, responseSize, this.nextEventsTime,this.is_multicast,multicast_destinations(hostRange, from));
                }
