...
public class MessageStatsReport extends Report implements MessageListener {
	...
	private static final int[] con = new int[10000] ;
	...
	public void messageTransferStarted(Message m, DTNHost from, DTNHost to) {
        if(from.connectionmake%10000==0&&from.connectionmake>0&&con[from.connectionmake/10000]==0)
        {
           con[from.connectionmake/10000] = 1;
           sahilconnection(from.connectionmake);
        }
	if (isWarmupID(m.getId())) {
		return;
	}
    ...
    /*
    Sahil: to print report after every regular interaction number
    */
public void sahilconnection(int x)
    {
    		write("\nNumber of intractions =  " + x + 
