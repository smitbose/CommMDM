...
public class Message implements Comparable<Message> {
	...
	private static boolean ttlAsSeconds = true;
	...
	public ArrayList<DTNHost> multicast_address = new ArrayList<DTNHost>();//sahil:multicast address features
	...
	public Message(DTNHost from, DTNHost to, String id, int size) {
       ...
       //Sahil: multicast feature
       	public Message(DTNHost from, DTNHost to, String id, int size,ArrayList<DTNHost> multicast_destinations) {
		this.from = from;
		this.to = to;
		this.id = id;
		this.size = size;
		...
		this.properties = null;
		this.appID = null;
		this.multicast_address = multicast_destinations;
                if(this.multicast_address.size()<5)
                {
                int x =10;
                }
		Message.nextUniqueId++;
	...
	protected void copyFrom(Message m) {
		...
		if (m.properties != null) {
            ...
            this.multicast_address = m.multicast_address;    
            }
            else
            {
            	this.path = new ArrayList<DTNHost>(m.path);
	...
	public Message replicate() {
            if(multicast_address.size()>0)
            {
               Message m = new Message(from, to, id, size,multicast_address);
	       m.copyFrom(this);
	       return m;  
            }
            else
